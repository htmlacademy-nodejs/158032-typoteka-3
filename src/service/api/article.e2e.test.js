...
describe(`Articles`, () => {
  ...
  describe(`Create new article`, () => {
    ...
    test(`Status code is CREATED`, async () => {
      await setup({
    ...
    test(`Articles count is increased by 1`, async () => {
      ...
      response = await request(app)
        .get(`/articles`);
      expect(response.body.length).toBe(articlesMockData.length + 1);
    });


    test(`Status code is BAD REQUEST when provided invalid data`, async () => {
      await setup({
  ...
  describe(`Delete an article`, () => {
    ...
    test(`Status code is NO CONTENT`, async () => {
      await setup({id: VALID_ARTICLE_ID});
      expect(response.status).toBe(HttpCode.NO_CONTENT);
    });

    test(`Articles count is reduced by 1`, async () => {
  ...
  describe(`Delete an article comment`, () => {
    ...
    test(`Status code is NO CONTENT`, async () => {
      await setup({id: VALID_ARTICLE_COMMENT_ID});
      expect(response.status).toBe(HttpCode.NO_CONTENT);
    });

    test(`Article comments count is reduced by 1`, async () => {
      const app = await setup({id: VALID_ARTICLE_COMMENT_ID});
      response = await request(app)
        .get(`/articles/${VALID_ARTICLE_ID}/comments`);
      expect(response.body.length).toBe(commentsMockData.length - 1);
    });
    ...
    test(`Article comment has been deleted`, async () => {
      ...
      response = await request(app)
        .get(`/articles/${VALID_ARTICLE_ID}/comments`);
      const foundArticleCommentsIds = response.body.map(({id}) => id);
